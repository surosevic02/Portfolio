...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_CTLCOLOREDIT:
            ...
            SetTextColor((HDC)wParam, textColor);
            static auto const solidBrush{ ::CreateSolidBrush(windowColor) };
            return reinterpret_cast<INT_PTR>(solidBrush);

        // Edit Button
        case WM_CTLCOLORBTN:
            SetBkColor((HDC)wParam, buttonColor);
            static auto const buttonBrush{ ::CreateSolidBrush(buttonColor) };
