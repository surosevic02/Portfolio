...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_COMMAND:
        {
            switch (LOWORD(wParam)) {
                ...
                case DELETE_BUTTON:
                {
                    ...
                    int idx = (int)SendMessageA(accountsDropdown, CB_GETCURSEL, 0, 0);
                    if (idx < 0) return 0;
        ...
        // Combobox List and Edit box
        case WM_CTLCOLORLISTBOX:
        case WM_CTLCOLOREDIT:
            SetBkColor((HDC)wParam, windowColor);
            SetTextColor((HDC)wParam, textColor);
            if (darkMode) {
                static auto const solidBrushDark{ ::CreateSolidBrush(RGB(25, 25, 25)) };
                return reinterpret_cast<INT_PTR>(solidBrushDark);
            }
            static auto const solidBrushLight{ ::CreateSolidBrush(RGB(255, 255, 255)) };
            return reinterpret_cast<INT_PTR>(solidBrushLight);

        // Edit Button
        case WM_CTLCOLORBTN:
            SetBkColor((HDC)wParam, buttonColor);
            if (darkMode) {
                static auto const buttonBrushDark{ ::CreateSolidBrush(RGB(25, 25, 25)) };
                return reinterpret_cast<INT_PTR>(buttonBrushDark);
            }
            static auto const buttonBrushLight{ ::CreateSolidBrush(buttonFaceColor) };
            return reinterpret_cast<INT_PTR>(buttonBrushLight);
        ...
        case WM_DESTROY:
            // This is where the application is closed out
